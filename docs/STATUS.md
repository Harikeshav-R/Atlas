...
**Phase 0 · AI provider abstraction.** Build `atlas.ai` per
...
1. `LLMProvider` protocol + `LLMRequest`/`LLMResponse` models (`src/atlas/ai/base.py`).
...
5. Capability probing, JSON-repair loop, failover chain, and `atlas doctor`.
...
| Phase | Title | State |
|---|---|---|
| 0 | Foundations (hygiene/CI · scaffold · config/DB/logging · AI providers) | 🚧 in progress — hygiene/CI + scaffold done; config/DB/logging + AI providers remain |
